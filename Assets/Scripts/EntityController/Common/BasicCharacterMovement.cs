...
public class BasicCharacterMovement : MoveObject {
    ...
    protected void OnTumbleEvent() {
        ...
        {
            EffectManager.CreateEffect("effect_tumble_back", transform.position, 0, null, !character.IsFacingRight());
            EffectManager.CreateEffect("effect_tumble_back2", transform.position, 0, null, !character.IsFacingRight());
            EffectManager.CreateEffect("effect_tumble_backf", transform.position, 0, transform, !character.IsFacingRight());
        ...
        {
            EffectManager.CreateEffect("effect_tumble_front", transform.position, 0, null, !character.IsFacingRight());
            EffectManager.CreateEffect("effect_tumble_front2", transform.position, 0, null, !character.IsFacingRight());
            EffectManager.CreateEffect("effect_tumble_frontf", transform.position, 0, transform, !character.IsFacingRight());
            ParticleManager.instance.CreateParticle("particle_tumb", transform.position, (int)Mathf.Sign(transform.localScale.x), transform);
			ParticleManager.instance.CreateParticle("particle_tumb2", transform.position, (int)Mathf.Sign(transform.localScale.x), transform);
    ...
    protected void OnParentedEffectEvent(string effect) {
        EffectManager.CreateEffect(effect, transform.position, 0, transform, !character.IsFacingRight());
    }

    /* 캐릭터와 독립된 이펙트 생성 (애니메이션 이벤트용) */
    protected void OnEffectEvent(string effect) {
        EffectManager.CreateEffect(effect, transform.position, 0, null, !character.IsFacingRight());
    }
    ...
    protected void OnThrowGrenade(string className) {
        ...
        BulletManager.CreateThrowable("throwable_grenade", throwpos, character, grenade,
