...
public class BulletManager : MonoBehaviour {
    ...
    public Bullet CreateBullet(string classname, Vector3 pos, Character user, Weapon firedfrom, float _speed, float angle, float _range, Dictionary<WeaponStats, float> _data) {
        ...
        bullet.Initialize(classname, user, firedfrom, _speed, _range, _data, false);
        return bullet;
    }

    public Bullet CreateBullet(string classname, Vector3 pos, Character user, Weapon firedfrom, float _speed, float angle, float _range, Dictionary<WeaponStats, float> _data, bool ignoreGround) {
        GameObject bullet_obj = (GameObject)Instantiate(Resources.Load("Prefab/Bullet"), pos, new Quaternion());
        Bullet bullet;
        if (GameDataManager.instance.GetData("Data", classname, "ScriptClass") != null && (string)GameDataManager.instance.GetData("Data", classname, "ScriptClass") != "") {
            bullet = (Bullet)bullet_obj.gameObject.AddComponent(Type.GetType((string)GameDataManager.instance.GetData("Data", classname, "ScriptClass")));
        ...
        ang.z = angle;
        bullet_obj.transform.eulerAngles = ang;
        bullet.Initialize(classname, user, firedfrom, _speed, _range, _data, ignoreGround);
        return bullet;
    }

    public Projectile CreateProjectile(string classname, Vector3 pos, Character user, Weapon firedfrom, float _speed, float angle, float _range, Dictionary<WeaponStats, float> _data, bool candirecthit) {
        ...
        projectile.Initialize(classname, user, firedfrom, _speed, _range, _data, candirecthit);
        return projectile;
    }

    public Throwable CreateThrowable(string classname, Vector3 pos, Character user, Weapon firedfrom, float _speed, float angle, float _range, float torque, Dictionary<WeaponStats, float> _data, bool candirecthit) {
        ...
        throwable.Initialize(classname, user, firedfrom, _speed, _range, torque, _data, candirecthit);
        return throwable;
    }
}
